...
template<>
std::vector<double> ConfigParser::getValueFrom( const std::string & parameterName ){
    return getVectorFromString(parameters[parameterName]);
}


std::vector<double> ConfigParser::increment( double start, double end, double incr ) const{
...
std::vector<double> ConfigParser::parseSubVector( const std::string & vec ) const{
    std::string copy = vec;
    std::vector<double> out;
    int colonCount = 0;
    double values[3];
...
template<>
std::vector<double> ConfigParser::getValueFrom( const char* parameterName ){
...
enum ConfigParser::StringType ConfigParser::getStringType( const std::string & str ){
